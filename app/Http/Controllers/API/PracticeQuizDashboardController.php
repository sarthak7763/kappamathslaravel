		...
		try{
	        ...
	        if($user)
	        {
	        	...
	        	if($quizresultmarks==null)
	        	{
	        		...
	        		if(count($subtopic_id) > 0)
	        		{

	        		$quiztopicsid=[];
	        		foreach ($subtopic_id as $key => $value) {
                	...
                	$total_score=$quizresultmarksarray['total_marks'];
        			...
        			if($attempt_question_ids_db!="")
        			{
	        			...
	        			foreach($attempt_question_ids_arr as $rowval)
	        			{
	        				foreach($rowval as $rowvalue)
	        				{
	        					$attempt_question_ids[]=$rowvalue;
	        				}
	        			}

	        			$questions_array_diff=array_values(array_diff($newrandom_question_ids, $attempt_question_ids));

        				$newfinalquestionid=$questions_array_diff[0];

        			}
        			else{
        				$newfinalquestionid=$newrandom_question_ids[0];
        			}

        			$currentquestionindex = array_keys($newrandom_question_ids,$newfinalquestionid);
	...
	public function submitpracticeobjectivequizquestion(Request $request)
	{
		try{
	        ...
	        if($user)
	        {
	        	...
	        	{
	        		$quiztopicdetaildata=$quiztopicdetail->toArray();
	        		$randomquizresultmarks=Resultmarks::where('user_id',$user->id)->whereRaw('"'.$result_date.'" between `result_marks_date` and `result_marks_end_date`')->whereRaw('FIND_IN_SET(?, topic_id)', $quizid)->where('subject',$courseid)->where('result_type','2')->get()->first();
	        		...
	        		if($randomquizresultmarks)
	        		{
	        			...
	        			{
	        				$data=$this->submitcorrectquizanswer($courseid,$user->id,$quiztopicdetaildata,$questiondetaildata,$result_date,$randomquizresultmarksarray,$previous_question_key,$next_question_key,$user_anwer,$previous_quiz_id,$next_quiz_id);
	        				if($data['status']==400)
	        				{
	        			...
	        			elseif($questiondetaildata['answer']!=$user_anwer){
	        				$data=$this->submitwrongquizanswer($courseid,$user->id,$quiztopicdetaildata,$questiondetaildata,$result_date,$randomquizresultmarksarray,$previous_question_key,$next_question_key,$user_anwer,$previous_quiz_id,$next_quiz_id);
	        				if($data['status']==400)
	        				{
	        					return $this::sendError('Unauthorised Exception.', ['error'=>$data['message']]);
	        				}
	        				...
	        				$data=$this->skipquizanswer($courseid,$user->id,$quiztopicdetaildata,$questiondetaildata,$result_date,$randomquizresultmarksarray,$previous_question_key,$next_question_key,$user_anwer,$previous_quiz_id,$next_quiz_id);
	        				if($data['status']==400)
	        				{
	        					return $this::sendError('Unauthorised Exception.', ['error'=>$data['message']]);
	        				}
	        	...
	        	if(count($subtopic_id) > 0)
	        	{
	        		...
	        		if(count($quiztopicsid) > 0)
	        		{
	        			...
	        			if(count($random_question_ids) > 0)
	        			{
                			...
                			$theoryquizresultupdate = Theoryquizresult::find($theoryquizresultid);
						          if(is_null($theoryquizresultupdate)){
						           return $this::sendError('Unauthorised Exception.', ['error'=>'Something went wrong']);
						        	...
						        	$theoryquizresultupdate->topic_id=$finalquiztopicids;
                			...
                			$questiondata=Question::where('id',$newfinalquestionid)->get()->first();
