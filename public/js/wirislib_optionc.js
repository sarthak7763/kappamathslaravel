 ...
 */
 function updateoptioncFunction() {
 	updatetextareaoptioncpreview();
 }

 function updateoptionclatexpreview() {
	...
	if(optionc_preview!="")
	{
		...
		mathhtmloptioncpreview = mathhtmloptioncpreview.replace(/<mspace linebreak="newline"\/>/g,'<mfenced open="{" close="}"><mspace/><mspace/></mfenced>');
    ...
    var previewlatex_div = document.getElementById("get_c_option_preview_latex");
	// Setting data on preview div.
	previewlatex_div.innerHTML = optionclatex;
}

 function updatetextareaoptioncpreview() {
	// Using plugin custom method for retreving data.
	// This data is a raw data with the format defined by save mode (xml, image or base64 images).
	var data = getOptioncEditorData();
	// This div simulates a render page without any editor.
	var preview_div = document.getElementById("get_c_option_preview");
	// Setting data on preview div.
	preview_div.innerHTML = data;
	// Rendering data on preview using JsPluginViewer.
	// Set titles for images. For demo purposes.
	imgOptioncSetTitle(preview_div);
}
 ...
 */
 function OptioncchangeMode(mode) {
	// Mathml mode.
	if (mode == 'xml') {
		saveMode = 'xml';
	}
 ...
 */
 function OptioncactivateTechLogo() {
	var wrs_tech = "php";
 ...
 *
 */
 function imgOptioncSetTitle(preview_div) {
...
try {
	...
	OptioncactivateTechLogo();	
   } catch (error) {
	console.log('demo activation', error);
} 
function Optionc_wrs_addEvent(element, event, func) {
...
Optionc_wrs_addEvent(window, 'load', function () {
	// Hide the textarea

	var textarea = document.getElementById('c_option');
	...
	Optionc_wrs_addEvent(iframe, 'load', function () {
 ...
 */
 function getOptioncEditorData() {
 	var iframe = document.getElementById('c_option_iframe');
 	return WirisPlugin.Parser.endParse(iframe.contentWindow.document.body.innerHTML);
 }


function getoptioncDataPreview(data) {
	return WirisPlugin.Parser.initParse(data);
}

function OptioncchangeDPI() {
...
function OptioncsetParametersSpecificPlugin(wiriseditorparameters) {
 ...
 */
 function getOptioncWirisEditorParameters() {
