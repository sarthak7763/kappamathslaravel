 ...
 */
 function updateoptiondFunction() {
 	updatetextareaoptiondpreview();
 }

 function updateoptiondlatexpreview() {
	...
	var optiond_preview = getOptiondEditorData();
	if(optiond_preview!="")
	{

		mathhtmloptiondpreview = optiond_preview.replace(/<mo[^>]*>&#xA0;<\/mo[^>]*>/g,'<mspace/>');
		mathhtmloptiondpreview = mathhtmloptiondpreview.replace(/<mo[^>]*>&#160;<\/mo[^>]*>/g,'<mspace/>');

		mathhtmloptiondpreview = mathhtmloptiondpreview.replace(/<mspace linebreak="newline"\/>/g,'<mfenced open="{" close="}"><mspace/><mspace/></mfenced>');
    ...
    var previewlatex_div = document.getElementById("get_d_option_preview_latex");
	// Setting data on preview div.
	previewlatex_div.innerHTML = optiondlatex;
}

 function updatetextareaoptiondpreview() {
	// Using plugin custom method for retreving data.
	// This data is a raw data with the format defined by save mode (xml, image or base64 images).
	var data = getOptiondEditorData();
	// This div simulates a render page without any editor.
	var preview_div = document.getElementById("get_d_option_preview");
	// Setting data on preview div.
	preview_div.innerHTML = data;
	// Rendering data on preview using JsPluginViewer.
	// Set titles for images. For demo purposes.
	imgOptiondSetTitle(preview_div);
}
 ...
 */
 function OptiondchangeMode(mode) {
	// Mathml mode.
	if (mode == 'xml') {
		saveMode = 'xml';
	}
 ...
 */
 function OptiondactivateTechLogo() {
	var wrs_tech = "php";
 ...
 *
 */
 function imgOptiondSetTitle(preview_div) {
...
try {
	...
	OptiondactivateTechLogo();	
   } catch (error) {
	console.log('demo activation', error);
} 
function Optiond_wrs_addEvent(element, event, func) {
...
Optiond_wrs_addEvent(window, 'load', function () {
	// Hide the textarea

	var textarea = document.getElementById('d_option');
	...
	Optiond_wrs_addEvent(iframe, 'load', function () {
 ...
 */
 function getOptiondEditorData() {
 	var iframe = document.getElementById('d_option_iframe');
 	return WirisPlugin.Parser.endParse(iframe.contentWindow.document.body.innerHTML);
 }


function getoptiondDataPreview(data) {
	return WirisPlugin.Parser.initParse(data);
}

function OptiondchangeDPI() {
...
function OptiondsetParametersSpecificPlugin(wiriseditorparameters) {
 ...
 */
 function getOptiondWirisEditorParameters() {
