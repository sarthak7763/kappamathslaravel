 ...
 */
 ...
 function updateoptionalatexpreview() {
	...
	var optiona_preview = getOptionaEditorData();
	if(optiona_preview!="")
	{
		var optionalatex = MathML2LaTeX.convert(optiona_preview);
	}
	else{
		var optionalatex="";
	}
 ...
 */
 function OptionachangeMode(mode) {
	// Mathml mode.
	if (mode == 'xml') {
		saveMode = 'xml';
	}
 ...
 */
 function OptionaactivateTechLogo() {
	var wrs_tech = "php";
 ...
 *
 */
 function imgOptionaSetTitle(preview_div) {
...
try {
	...
	OptionaactivateTechLogo();	
   } catch (error) {
	console.log('demo activation', error);
} 
function Optiona_wrs_addEvent(element, event, func) {
...
Optiona_wrs_addEvent(window, 'load', function () {
	// Hide the textarea

	var textarea = document.getElementById('a_option');
	...
	Optiona_wrs_addEvent(iframe, 'load', function () {
 ...
 */
 function getOptionaEditorData() {
 	var iframe = document.getElementById('a_option_iframe');
 	return WirisPlugin.Parser.endParse(iframe.contentWindow.document.body.innerHTML);
 }


function getoptionaDataPreview(data) {
	return WirisPlugin.Parser.initParse(data);
}

function OptionachangeDPI() {
...
function OptionasetParametersSpecificPlugin(wiriseditorparameters) {
 ...
 */
 function getOptionaWirisEditorParameters() {
